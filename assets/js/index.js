...
function getDistance(positions) {
  ...
  for (let index = 0; index < positions.length - 1; index++) {
    ...
    const p2 = {
      latitude: positions[index + 1].latitude,
      longitude: positions[index + 1].longitude,
    };
...
function getStartDate(ride) {
  ...
  const hour = d.toLocaleString("en-US", { hour: "2-ditits", hour12: false });
  const minute = d.toLocaleString("en-US", { minute: "2-digits" });
